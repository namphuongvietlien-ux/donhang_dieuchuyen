...
var danhMucGoc = {}; var danhMucArr = []; var danhMucByMaHang_ = {}; var danhMucByMaVach_ = {}; var danhMucByKey_ = {};
var danhMucByMaHangList_ = {};
var arrItems = []; var gStores = [];
var storeMap = {};
var storeDetails = [];
var storeDetailsByKey_ = {};
var newProductsList = [];
...
/** Clone nhẹ field sản phẩm đưa vào giỏ — tránh mutate object catalog dùng chung */
function cloneCatalogProduct_(src) {
  if (!src) return null;
  return {
    key: src.key || '',
    maHang: String(src.maHang == null ? '' : src.maHang),
    maVach: String(src.maVach == null ? '' : src.maVach),
    tenHang: src.tenHang || '',
    dvt: src.dvt || '',
    dvt2: src.dvt2 || '',
...
/** Composite Key FE: MaSP_ĐVT[_MaVach] — mirror backend */
function normalizeCatalogDvtPartClient_(dvt) {
...
function buildCatalogCompositeKeyClient_(maHang, dvt, maVach, includeMaVach) {
  ...
  var base = mh + '_' + normalizeCatalogDvtPartClient_(dvt);
  if (includeMaVach && mv) return base + '_' + mv;
  return base;
}

function ensureItemCatalogKey_(it) {
  if (!it) return '';
  if (it.key) return String(it.key);
  it.key = buildCatalogCompositeKeyClient_(it.maHang, it.dvt, it.maVach, false);
  return it.key;
}

/** So khớp mã SP/vạch 1:1 sau chuẩn hóa NFC (không substring) */
function cartCodesEqual_(a, b) {
  var na = normalizeProductCodeClient_(a);
  var nb = normalizeProductCodeClient_(b);
  if (na && nb) return na === nb;
  return String(a || '').trim().toUpperCase() === String(b || '').trim().toUpperCase();
}

function ensureCartLineId_(it) {
  if (!it) return '';
  if (!it.lineId) {
    it.lineId = 'ln-' + Date.now().toString(36) + '-' + Math.floor(Math.random() * 1e6).toString(36);
  }
  return String(it.lineId);
}

function findCartItemByLineId_(list, lineId) {
  var id = String(lineId || '');
  if (!id || !list || !list.length) return -1;
  for (var i = 0; i < list.length; i++) {
    if (list[i] && String(list[i].lineId || '') === id) return i;
  }
  return -1;
}

/** Danh sách ĐVT hợp lệ — mọi quy cách cùng MaSP (Composite Key) */
function getCatalogDvtOptions_(maHang, maVach) {
  var opts = [];
  var seen = {};
  function pushDvt(v) {
    var s = String(v || '').trim();
  ...
  var mh = normalizeProductCodeClient_(maHang);
  var list = (mh && danhMucByMaHangList_[mh]) ? danhMucByMaHangList_[mh] : [];
  if (list && list.length) {
    for (var i = 0; i < list.length; i++) pushDvt(list[i].dvt);
  } else {
...
/**
 * Tra cứu 1:1 theo Composite Key (MaSP_ĐVT[_MaVach]).
 * Không dùng index mảng filter. Có thể truyền key trực tiếp qua tham số thứ 3 (dvtOrKey).
 */
function resolveSelectedCatalogProduct_(maHang, maVach, dvtOrKey) {
  var mhRaw = String(maHang || '').trim();
  var mvRaw = String(maVach || '').trim();
  var third = String(dvtOrKey || '').trim();
  var mhU = normalizeProductCodeClient_(mhRaw) || mhRaw.toUpperCase();
  var mvU = normalizeProductCodeClient_(mvRaw) || mvRaw.toUpperCase();
  var found = null;

  // 0) Key đầy đủ truyền thẳng
  if (third && danhMucByKey_[third]) found = danhMucByKey_[third];
  if (!found && third && danhMucGoc[third] && danhMucGoc[third].key) found = danhMucGoc[third];

  // 1) Composite Key MaSP_ĐVT[_MV]
  if (!found && mhU && third && third.indexOf('_') === -1) {
    var k1 = buildCatalogCompositeKeyClient_(mhU || mhRaw, third, mvRaw, false);
    var k2 = buildCatalogCompositeKeyClient_(mhU || mhRaw, third, mvRaw, true);
    found = danhMucByKey_[k1] || danhMucByKey_[k2] || null;
  }

  // 2) MaSP + MaVach + ĐVT trong list quy cách
  if (!found && mhU && danhMucByMaHangList_[mhU]) {
    var list = danhMucByMaHangList_[mhU];
    var both = null;
    var dvtMatch = null;
    var mhOnly = null;
    for (var i = 0; i < list.length; i++) {
      ...
      var dvtOk = !third || third.indexOf('_') !== -1 ||
        normalizeCatalogDvtPartClient_(it.dvt) === normalizeCatalogDvtPartClient_(third);
      if (mvU && itMv && itMv === mvU && dvtOk) { both = it; break; }
      if (dvtOk && !dvtMatch) dvtMatch = it;
      if (!mhOnly) mhOnly = it;
    }
    found = both || dvtMatch || mhOnly;
  }
  ...
  // 4) Fallback map
  if (!found && mhU && danhMucByMaHang_[mhU]) found = danhMucByMaHang_[mhU];
...
function resolveSelectedCatalogProductByKey_(keyEnc) {
  var key = String(keyEnc || '');
  try { key = decodeURIComponent(key); } catch (e) {}
  key = String(key || '').trim();
  if (!key) return null;
  var found = danhMucByKey_[key] || (danhMucGoc[key] && danhMucGoc[key].key ? danhMucGoc[key] : null);
  if (found) return cloneCatalogProduct_(found);
  // key có thể là MaSP_dvt — parse
  var parts = key.split('_');
  if (parts.length >= 2) {
    var mh = parts[0];
    var dvt = parts[1];
    var mv = parts.length >= 3 ? parts.slice(2).join('_') : '';
    return resolveSelectedCatalogProduct_(mh, mv, dvt);
  }
  return null;
}
 ...
 * Kết quả cuối luôn resolve lại theo Composite Key.
 */
function resolveProductFromScanQuery_(rawQuery) {
  ...
  if (code && danhMucByKey_[code]) return cloneCatalogProduct_(danhMucByKey_[code]);
  if (code && danhMucByMaVach_[code]) {
    return cloneCatalogProduct_(danhMucByMaVach_[code]);
  }
  if (code && danhMucByMaHangList_[code] && danhMucByMaHangList_[code].length === 1) {
    return cloneCatalogProduct_(danhMucByMaHangList_[code][0]);
  }
  if (code && danhMucByMaHang_[code]) {
    return cloneCatalogProduct_(danhMucByMaHang_[code]);
  }
  var matched = filterProducts(q);
  if (matched && matched.length) {
    return resolveSelectedCatalogProductByKey_(matched[0].key) ||
      resolveSelectedCatalogProduct_(matched[0].maHang, matched[0].maVach, matched[0].dvt);
  }
  return null;
}
...
var productLockedFocusInputId_ = 'input-scan';

function isProductLockedFlag_(val) {
  if (val === true || val === 1) return true;
  var s = String(val == null ? '' : val).trim().toLowerCase();
  return s === '1' || s === 'true' || s === 'yes' || s === 'x' || s === 'locked' || s === 'khoa';
}

/** true nếu mã bị khóa đặt hàng (catalog / item.isLocked) */
function isProductLockedItem_(it) {
  if (!it) return false;
...
function closeProductLockedModal_() {
  var modal = document.getElementById('modal-product-locked');
  ...
  var suggestMap = {
    'input-scan': 'suggest-box',
    'ql-add-code': 'ql-suggest-box',
    'xb-input-scan': 'xb-suggest-box'
  };
  var boxId = suggestMap[focusId];
  var box = boxId ? document.getElementById(boxId) : null;
  if (box) box.style.display = 'none';
}

function warnProductLocked_(it, focusInputId) {
  productLockedFocusInputId_ = focusInputId || 'input-scan';
  var ma = String((it && (it.maHang || it.maVach)) || '').trim() || '?';
  var ten = String((it && it.tenHang) || '').trim() || '';
  var msg = 'Mã sản phẩm [' + ma + (ten ? (' - ' + ten) : '') + '] đã bị KHÓA đặt hàng. Vui lòng chọn mã khác!';
  var msgEl = document.getElementById('product-locked-msg');
  ...
  if (modal) {
    ...
    alert(msg);
    closeProductLockedModal_();
  }
  return false;
}

/** Chặn thêm vào giỏ nếu bị khóa — trả false khi đã cảnh báo */
function guardProductNotLocked_(it, focusInputId) {
  if (!isProductLockedItem_(it)) return true;
  warnProductLocked_(it, focusInputId);
  return false;
}
...
/**
 * ĐVT hiển thị/lưu: ưu tiên ĐVT user đã chọn (selectedDVT / currentDvt)
 * nếu khớp ĐVT1 hoặc ĐVT2 trên catalog — không ghi đè về ĐVT1.
 */
function resolveDvtClient_(maHang, maVach, currentDvt) {
  var cur = String(currentDvt || '').trim();
  var opts = getCatalogDvtOptions_(maHang, maVach);
  if (cur) {
    for (var i = 0; i < opts.length; i++) {
      if (String(opts[i]).trim().toUpperCase() === cur.toUpperCase()) return opts[i];
    }
...
/** HTML select ĐVT1/ĐVT2 trên Bảng Tạm — cập nhật state qua onChangeUnit */
function buildCartDvtSelectHtml_(it, lineId, onChangeFnName) {
  var opts = getCatalogDvtOptions_(it && it.maHang, it && it.maVach);
  var cur = String((it && (it.selectedDVT || it.dvt)) || '').trim();
  if (cur) {
    var hasCur = false;
    for (var i = 0; i < opts.length; i++) {
      if (String(opts[i]).toUpperCase() === cur.toUpperCase()) {
        hasCur = true;
        cur = opts[i];
        break;
      }
    }
    if (!hasCur) opts = [cur].concat(opts);
  }
  if (!opts.length) return '—';
  if (opts.length === 1) return escapeHtml(opts[0]);
  var fn = onChangeFnName || 'onChangeUnit';
  var html = '<select class="cart-dvt-select" style="max-width:110px;padding:4px 6px;border-radius:8px;border:1px solid #cbd5e1;" onchange="' +
    fn + '(\'' + String(lineId).replace(/'/g, '') + '\', this.value)">';
  for (var j = 0; j < opts.length; j++) {
    var o = opts[j];
    var sel = String(o).toUpperCase() === String(cur).toUpperCase() ? ' selected' : '';
...
function rebuildCatalogArray() {
  ...
  for (var i = 0; i < danhMucArr.length; i++) {
    var it = danhMucArr[i];
    if (!it) continue;
    var ck = ensureItemCatalogKey_(it);
    if (ck) danhMucByKey_[ck] = it;
    ...
    if (mhU) {
      danhMucByMaHang_[mhU] = it;
      if (!danhMucByMaHangList_[mhU]) danhMucByMaHangList_[mhU] = [];
      danhMucByMaHangList_[mhU].push(it);
    }
    if (mhRaw && mhRaw !== mhU) {
      danhMucByMaHang_[mhRaw] = it;
      if (!danhMucByMaHangList_[mhRaw]) danhMucByMaHangList_[mhRaw] = [];
      danhMucByMaHangList_[mhRaw].push(it);
    }
...
function rebuildVariantIndex_() {
  ...
  danhMucArr.forEach(function(item) {
    ...
    var dedupe = (normalizeProductCodeClient_(item.maHang) || '') + '|' +
      (normalizeProductCodeClient_(item.maVach) || '') + '|' +
      normalizeCatalogDvtPartClient_(item.dvt);
    var exists = variantIndexByParent[p].some(function(v) {
      return (normalizeProductCodeClient_(v.maHang) || '') + '|' +
        (normalizeProductCodeClient_(v.maVach) || '') + '|' +
        normalizeCatalogDvtPartClient_(v.dvt) === dedupe;
    });
...
function applyBootstrapData(res) {
  ...
  storeDetails = Array.isArray(res.storeDetails) ? res.storeDetails : [];
  storeDetailsByKey_ = {};
  for (var si = 0; si < storeDetails.length; si++) {
    var d = storeDetails[si];
    if (!d) continue;
    var full = String(d.fullName || '').trim();
    var shortN = String(d.shortName || '').trim();
    if (full) storeDetailsByKey_[full] = d;
    if (shortN) storeDetailsByKey_[shortN] = d;
    var code = String(d.code || '').trim();
    if (code) storeDetailsByKey_[code] = d;
  }
...
function renderNewProductsHighlight() {
  ...
  var cardsHtml = newProductsList.map(function(item, idx) {
    ...
    return '' +
      '<button type="button" class="new-product-card" onclick="switchTab(\'tab-tao-phieu\'); chonSanPhamByMaSP_(\'' + mhEnc + '\',\'' + mvEnc + '\');">' +
...
function addNewProductToOrder(encodedItem) {
  try {
    ...
    chonSanPhamByMaSP_(
      encodeURIComponent(String(item.maHang || '')),
      encodeURIComponent(String(item.maVach || ''))
    );
    var input = document.getElementById('input-scan');
    if (input) {
      input.value = '';
...
function hidePasswordSection() {
  // form đổi MK nằm trong Admin > Cài đặt — không cần ẩn card riêng
}
...
function showPasswordSection() {
  ...
  if (document.getElementById('tab-admin')) {
    switchTab('tab-admin');
    switchAdminSubTab_('adm-tab-settings');
  }
...
function switchAdminSubTab_(panelId) {
  var panels = ['adm-tab-catalog', 'adm-tab-sync', 'adm-tab-orders', 'adm-tab-settings'];
  var mapBtn = {
    'adm-tab-catalog': 'adm-subtab-catalog',
    'adm-tab-sync': 'adm-subtab-sync',
    'adm-tab-orders': 'adm-subtab-orders',
    'adm-tab-settings': 'adm-subtab-settings'
  };
  if (panels.indexOf(panelId) === -1) panelId = 'adm-tab-catalog';
  panels.forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.classList.toggle('active', id === panelId);
  });
  Object.keys(mapBtn).forEach(function(pid) {
    var btn = document.getElementById(mapBtn[pid]);
    if (btn) btn.classList.toggle('active', pid === panelId);
  });
  if (panelId === 'adm-tab-orders') loadDSUser();
  if (panelId === 'adm-tab-settings') {
    var bl = document.getElementById('adm-build-label');
...
function loadDashboardSummary() {
  ...
  apiGet('getDashboardSummary', { userRole: sessionUser.role || '', userStore: sessionUser.store || '', timeline: timeline, fromDate: timelineParams.fromDate, toDate: timelineParams.toDate }).then(function(res) {
    ...
    var rows = data.recentOrders.map(function(order) {
      ...
      var created = formatOrderTimestampUiClient_(order.thoiGianTao || order.thoiGian) || order.thoiGian || '-';
      var updated = formatOrderTimestampUiClient_(order.thoiGianCapNhat) || created;
      return '<tr><td><a href="javascript:void(0)" class="order-pdf-link" onclick="openOrderPdfView(\'' + soAttr + '\')">' + escapeHtml(so) + '</a></td><td>' + kx + '</td><td>' + kn + '</td><td><span style="display:inline-block; padding:4px 8px; border-radius:999px; background:#eff6ff; color:#1d4ed8; font-size:12px; font-weight:700;">' + order.status + '</span></td><td style="font-size:12px;line-height:1.35;"><div>Tạo: ' + escapeHtml(created) + '</div><div style="color:#64748b;">Sửa: ' + escapeHtml(updated) + '</div></td></tr>';
    }).join('');

    recent.innerHTML = '<div style="overflow-x:auto;"><table style="width:100%; border-collapse:collapse;"><thead><tr><th style="text-align:left; padding:8px; border-bottom:1px solid #e2e8f0;">Số phiếu</th><th style="text-align:left; padding:8px; border-bottom:1px solid #e2e8f0;">Kho xuất</th><th style="text-align:left; padding:8px; border-bottom:1px solid #e2e8f0;">Kho nhận</th><th style="text-align:left; padding:8px; border-bottom:1px solid #e2e8f0;">Trạng thái</th><th style="text-align:left; padding:8px; border-bottom:1px solid #e2e8f0;">Ngày tạo / Lần sửa</th></tr></thead><tbody>' + rows + '</tbody></table></div>';
  }).catch(function(err){ hideLoad(); console.error(err); });
}

/** Tên ngắn kho thống nhất trên Web (Q4 cho cả 178/275) */
...
function lookupStoreAddressClient_(storeName) {
  var raw = String(storeName || '').trim();
  if (!raw) return '';
  if (storeDetailsByKey_[raw] && storeDetailsByKey_[raw].address) {
    return String(storeDetailsByKey_[raw].address || '').trim();
  }
  var fold = raw.toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g, '').replace(/đ/g, 'd').replace(/[^a-z0-9]+/g, '');
  var codeHint = '';
  if (fold.indexOf('178') !== -1 || (fold.indexOf('q4') !== -1 && fold.indexOf('moi') !== -1)) codeHint = '178';
  if (fold.indexOf('275') !== -1 || (fold.indexOf('q4') !== -1 && (fold.indexOf('cu') !== -1 || fold.indexOf('old') !== -1))) codeHint = '275';
  if (PACKING_STORE_CODES_[raw]) codeHint = PACKING_STORE_CODES_[raw];
  for (var i = 0; i < (storeDetails || []).length; i++) {
    var d = storeDetails[i];
    if (!d) continue;
    if (String(d.fullName || '').trim() === raw || String(d.shortName || '').trim() === raw) {
...
/** Mã ngắn phân biệt 2 kho Q4 — chỉ dùng cột bảng soạn (không hiện trên UI tên chi nhánh) */
var PACKING_STORE_CODES_ = {
  'Kho Địa điểm kinh doanh 01': '178',
  'Kho Địa điểm kinh doanh 06': '275'
};

/** Ô kho trên bảng soạn — tên ngắn thống nhất */
function formatPackingStoreCellHtml_(storeName) {
  var short = formatStoreShortLabel_(storeName);
  return '<span class="sh-store-name">' + escapeHtml(short) + '</span>';
}
...
function applyQuyenKho() {
  ...
  xb_applyStoreQuyen_();
  try { refreshCreateOrderMetaBadge_(); } catch (eMeta) {}
}
...
function handleSearchInput(e) {
  ...
  if (e && e.key === "Enter") {
    ...
    if (exactMatch) {
      chonSanPhamByKey_(encodeURIComponent(String(exactMatch.key || ensureItemCatalogKey_(exactMatch))));
    } else {
  ...
  results.slice(0, 12).forEach(function(item) {
    ...
    html += '<div class="suggest-item" onclick="chonSanPhamByKey_(\'' + keyEnc + '\')">' +
      ...
      ' · <b>Mã vạch:</b> ' + escapeHtml(item.maVach || '—') +
      ' · <b>Tồn:</b> ' + escapeHtml(tonLabel) +
      (item.parentSku ? (' · Parent: ' + escapeHtml(item.parentSku)) : '') +
      '</div></div>';
  });
...
function filterProducts(kw) {
  ...
  return scored.filter(function(entry) {
    var item = entry.item;
    var key = String(item.key || ensureItemCatalogKey_(item));
...
function ql_handleAddCodeInput(e) {
  ...
  results.slice(0, 8).forEach(function(item) {
    ...
    html += '<div class="suggest-item" onclick="ql_pickSuggestedItemByKey_(\'' + keyEnc + '\')">' +
      ...
      '<div class="sg-desc"><b>ĐVT:</b> ' + escapeHtml(item.dvt || '—') +
      ' · <b>Mã vạch:</b> ' + escapeHtml(item.maVach || '—') +
      ' · <b>Tồn:</b> ' + escapeHtml(tonLabel) + '</div></div>';
  });
  box.innerHTML = html;
  box.style.display = "block";
}
...
function ql_pickSuggestedItem(itemStr) {
  // Legacy: JSON payload — resolve lại theo MaSP
  try {
    var parsed = JSON.parse(decodeURIComponent(itemStr));
    ql_pickSuggestedItemByMaSP_(
      encodeURIComponent(String(parsed.maHang || '')),
      encodeURIComponent(String(parsed.maVach || ''))
    );
  } catch (e) {}
}

function ql_pickSuggestedItemByMaSP_(maHangEnc, maVachEnc) {
  var mh = decodeURIComponent(String(maHangEnc || ''));
  var mv = decodeURIComponent(String(maVachEnc || ''));
  var item = resolveSelectedCatalogProduct_(mh, mv);
  if (!item) return alert('Không tìm thấy sản phẩm [' + (mh || mv || '?') + '] trong danh mục.');
...
function ql_pickSuggestedItemByKey_(keyEnc) {
  var item = resolveSelectedCatalogProductByKey_(keyEnc);
  if (!item) return alert('Không tìm thấy quy cách đã chọn trong danh mục.');
  var input = document.getElementById("ql-add-code");
  var box = document.getElementById("ql-suggest-box");
  var qtyInput = document.getElementById("ql-add-qty");
  qlSelectedSuggestedItem = item;
...
/** Chọn từ gợi ý theo Composite Key (MaSP_ĐVT[_MaVach]) */
function chonSanPhamByKey_(keyEnc) {
  var product = resolveSelectedCatalogProductByKey_(keyEnc);
  if (!product) {
    alert('Không tìm thấy quy cách trong danh mục. Hãy tải lại danh mục.');
    return;
  }
  chonSanPham(product);
  var input = document.getElementById("input-scan");
...
/** Compat: MaSP + MaVach → resolve quy cách (ưu tiên khớp cả hai) */
function chonSanPhamByMaSP_(maHangEnc, maVachEnc) {
  var mh = decodeURIComponent(String(maHangEnc || ''));
  var mv = decodeURIComponent(String(maVachEnc || ''));
  var product = resolveSelectedCatalogProduct_(mh, mv);
  if (!product) {
    alert('Không tìm thấy sản phẩm [' + (mh || mv || '?') + '] trong danh mục. Hãy tải lại danh mục.');
    return;
  }
  chonSanPham(product);
  var input = document.getElementById("input-scan");
...
function chonSanPhamFromSuggest(itemStr) {
  ...
  try {
    var parsed = JSON.parse(decodeURIComponent(itemStr));
    if (parsed.key) {
      chonSanPhamByKey_(encodeURIComponent(String(parsed.key)));
      return;
    }
    chonSanPhamByMaSP_(
      encodeURIComponent(String(parsed.maHang || '')),
    ...
    alert('Không đọc được sản phẩm đã chọn.');
  }
}
...
function openVariantPicker_(seedItem, mode, qty) {
  ...
  function renderRows_(variants) {
    ...
    variants.forEach(function(v, idx) {
      ...
      var mhAttr = encodeURIComponent(String(v.maHang || ''));
      var mvAttr = encodeURIComponent(String(v.maVach || ''));
      html += '<tr class="variant-pick-row' + zeroClass + '" data-ma-hang="' + mhAttr + '" data-ma-vach="' + mvAttr + '">' +
        ...
        '<td>' + stockHtml + '</td>' +
        '<td><input type="number" class="vp-qty" id="vp-qty-' + idx + '" data-ma-hang="' + mhAttr + '" data-ma-vach="' + mvAttr + '" min="0" step="1" value="0" inputmode="numeric" onclick="event.stopPropagation()"></td>' +
        '</tr>';
    });
    html += '</tbody></table>';
    list.innerHTML = html;
...
/** Gom các dòng có SL > 0 từ modal → đẩy vào đơn (resolve theo MaSP, không tin index) */
function confirmVariantPicker_() {
  ...
  for (var i = 0; i < variants.length; i++) {
    ...
    var mhFromEl = qtyEl ? decodeURIComponent(String(qtyEl.getAttribute('data-ma-hang') || '')) : '';
    var mvFromEl = qtyEl ? decodeURIComponent(String(qtyEl.getAttribute('data-ma-vach') || '')) : '';
    var v = variants[i] || {};
    var mh = mhFromEl || v.maHang || '';
    var mv = mvFromEl || v.maVach || '';
    var resolved = resolveSelectedCatalogProduct_(mh, mv) || cloneCatalogProduct_(v);
    if (!resolved) continue;
    picked.push({
      maHang: resolved.maHang || mh,
      maVach: resolved.maVach || mv,
      tenHang: resolved.tenHang || v.tenHang || '',
      dvt: resolved.dvt || v.dvt || '',
      parentSku: resolved.parentSku || v.parentSku || variantPickerState.parentSku || '',
      sl: qty
    });
  }
...
function chonSanPhamDirect_(it, qty) {
  if (!it) return;
  // Resolve theo Composite Key (hoặc MaSP+ĐVT+MV)
  var resolved = null;
  if (it.key) resolved = resolveSelectedCatalogProductByKey_(it.key);
  if (!resolved) {
    resolved = resolveSelectedCatalogProduct_(it.maHang, it.maVach, it.dvt || it.selectedDVT) ||
      cloneCatalogProduct_(it) || it;
  }
  ...
  var pickDvt = String(resolved.dvt || it.selectedDVT || it.dvt || '').trim();
  var resolvedKey = resolved.key || ensureItemCatalogKey_(resolved);
  var existingIndex = arrItems.findIndex(function(x) {
    if (x.maHang === "LỖI MÃ") return false;
    var xKey = x.key || buildCatalogCompositeKeyClient_(x.maHang, x.selectedDVT || x.dvt, x.maVach, false);
    if (resolvedKey && xKey && resolvedKey === xKey) return true;
    return cartCodesEqual_(x.maHang, resolved.maHang) &&
      cartCodesEqual_(x.maVach, resolved.maVach) &&
      normalizeCatalogDvtPartClient_(x.selectedDVT || x.dvt) === normalizeCatalogDvtPartClient_(pickDvt);
  });
  if (existingIndex !== -1) {
    arrItems[existingIndex].sl = Number(arrItems[existingIndex].sl) + addQty;
    arrItems[existingIndex].highlight = true;
    arrItems[existingIndex].tenHang = resolved.tenHang || arrItems[existingIndex].tenHang;
    arrItems[existingIndex].dvt = pickDvt || arrItems[existingIndex].dvt;
    arrItems[existingIndex].selectedDVT = arrItems[existingIndex].dvt;
    arrItems[existingIndex].key = resolvedKey;
    arrItems[existingIndex].maVach = String(resolved.maVach || arrItems[existingIndex].maVach || '');
    if (resolved.parentSku) arrItems[existingIndex].parentSku = resolved.parentSku;
    ensureCartLineId_(arrItems[existingIndex]);
  } else {
    var row = {
      lineId: '',
      key: resolvedKey,
      maHang: String(resolved.maHang || ''),
      maVach: String(resolved.maVach || ''),
      tenHang: resolved.tenHang,
      dvt: pickDvt,
      selectedDVT: pickDvt,
      tonKho: resolved.tonKho,
      parentSku: resolved.parentSku || '',
      sl: String(addQty),
      highlight: true
    };
    ensureCartLineId_(row);
    arrItems.unshift(row);
  }
  renderTable();
}
...
function thayDoiSoLuong(lineId, delta) {
  var index = findCartItemByLineId_(arrItems, lineId);
  if (index < 0) return;
  var currentSl = Number(arrItems[index].sl) || 0;
  var newSl = currentSl + delta;
  if (newSl > 0) { arrItems[index].sl = newSl; renderTable(); }
}

function setCartQtyByLineId_(lineId, value) {
  var index = findCartItemByLineId_(arrItems, lineId);
  if (index < 0) return;
  arrItems[index].sl = value;
  renderTable();
}

function removeCartItemByLineId_(lineId) {
  var index = findCartItemByLineId_(arrItems, lineId);
  if (index < 0) return;
...
/** Đổi ĐVT trên Bảng Tạm — chuyển sang đúng quy cách Composite Key */
function onChangeUnit(lineId, newDvt) {
  var index = findCartItemByLineId_(arrItems, lineId);
  if (index < 0) return;
  var dvt = String(newDvt || '').trim();
  var cur = arrItems[index];
  var match = resolveSelectedCatalogProduct_(cur.maHang, '', dvt) ||
    resolveSelectedCatalogProduct_(cur.maHang, cur.maVach, dvt);
  if (match) {
    arrItems[index].key = match.key || buildCatalogCompositeKeyClient_(match.maHang, match.dvt, match.maVach, false);
    arrItems[index].maHang = String(match.maHang || cur.maHang || '');
    arrItems[index].maVach = String(match.maVach || cur.maVach || '');
    arrItems[index].tenHang = match.tenHang || cur.tenHang;
    arrItems[index].dvt = String(match.dvt || dvt);
    arrItems[index].selectedDVT = arrItems[index].dvt;
    if (match.tonKho != null) arrItems[index].tonKho = match.tonKho;
  } else {
    arrItems[index].dvt = dvt;
    arrItems[index].selectedDVT = dvt;
    arrItems[index].key = buildCatalogCompositeKeyClient_(cur.maHang, dvt, cur.maVach, false);
  }
}

function renderTable() {
  var tbody = document.getElementById("tbody-items"); tbody.innerHTML = ""; var tongSl = 0;
  arrItems.forEach(function(it, i) {
    var lineId = ensureCartLineId_(it);
    ...
    var dvtResolved = resolveDvtClient_(it.maHang, it.maVach, it.selectedDVT || it.dvt);
    it.dvt = dvtResolved || it.dvt;
    it.selectedDVT = it.dvt;
    var dvtCell = isErr ? escapeHtml(it.dvt || '—') : buildCartDvtSelectHtml_(it, lineId, 'onChangeUnit');
    var tenShow = formatPackingProductName_(it);
    var lid = String(lineId).replace(/\\/g, '\\\\').replace(/'/g, "\\'");
    tbody.insertAdjacentHTML('beforeend', '<tr class="' + trClass + '" data-line-id="' + escapeHtml(lineId) + '"><td>' + (arrItems.length - i) + '</td><td>' + formatPackingCodeHtml_(it) + '</td><td style="font-weight:500;">' + escapeHtml(tenShow) + newBadge + outBadge + groupBadge + '</td><td>' + dvtCell + '</td><td><div class="qty-control"><button class="qty-btn" onclick="thayDoiSoLuong(\'' + lid + '\', -1)">-</button><input type="number" class="qty-input" value="' + it.sl + '" onchange="setCartQtyByLineId_(\'' + lid + '\', this.value)"><button class="qty-btn" onclick="thayDoiSoLuong(\'' + lid + '\', 1)">+</button></div></td><td style="text-align:center;"><button style="color:#d93025; border:none; background:none; font-weight:bold; cursor:pointer; font-size:18px;" onclick="removeCartItemByLineId_(\'' + lid + '\')">×</button></td></tr>');
  });
...
function submitPhieuMoi() {
  ...
  var itemsToSave = arrItems.map(function(it) {
    ...
    return {
      maHang: String(it.maHang || ''),
      maVach: String(it.maVach || ''),
      tenHang: it.tenHang,
      dvt: dvtSave,
      sl: it.sl,
...
function submitPhieuMoiDoSave_(lPhieu, khoXuat, khoNhan, itemsToSave) {
  ...
  apiPost('luuPhieuTuWebApp', { loaiPhieu: lPhieu, khoXuat: khoXuat, khoNhan: khoNhan, items: itemsToSave }).then(function(res) {
    ...
    if(res.coLoi) { alert("⚠️ Có mã lỗi. Sửa trong tab Quản lý!"); arrItems = []; renderTable(); }
       ...
       currentPhieuObj = {
         ...
         khoXuat: khoXuat,
         khoNhan: khoNhan,
         thoiGianPretty: formatOrderTimestampUiClient_(new Date()),
         thoiGianTao: formatOrderTimestampUiClient_(new Date()),
         thoiGianCapNhat: formatOrderTimestampUiClient_(new Date())
       };
       document.getElementById("modal-sophieu").innerText = res.soPhieu;
       setOrderMetaBadgeEl_('modal-order-meta', res.soPhieu, khoNhan, khoXuat, currentPhieuObj.thoiGianTao, currentPhieuObj.thoiGianCapNhat);
  ...
  }).catch(function(err){ hideLoad(); endSubmitBusy_('submitPhieuMoi'); alert('Lỗi: '+err.message); });
}
...
function getPdfWindowSharedCss_() {
  return '' +
    ...
    '.created-time{margin:0 0 14px;font-size:14px;font-weight:700;color:#1d4ed8;line-height:1.5;}' +
    ...
    '@media print{.toolbar{display:none!important;} body{background:#fff;padding:0;} .sheet{box-shadow:none;border:none;border-radius:0;max-width:none;padding:0;page-break-after:always;} .sheet:last-child{page-break-after:auto;} @page{margin:10mm;size:auto;}}' +
    '@media (max-width:600px){body{padding:12px;} .sheet{padding:16px;} .btn-submit{width:100%;}}';
}

/** Giờ tạo đơn dạng 10:15 - 05/08/2026 (tương thích cũ) */
function formatOrderCreatedAtPrettyClient_(value) {
  ...
  if (value instanceof Date) d = value;
    ...
    if (mVn) {
      ...
      var mUi = s.match(/^(\d{1,2}):(\d{2})\s+(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
      if (mUi) return pad2_(Number(mUi[1])) + ':' + pad2_(Number(mUi[2])) + ' - ' +
        pad2_(Number(mUi[3])) + '/' + pad2_(Number(mUi[4])) + '/' + mUi[5];
      d = new Date(s);
    }
  }
...
/** Nhãn UI: HH:mm dd/MM/yyyy (vd: 17:32 04/08/2026) */
function formatOrderTimestampUiClient_(value) {
  if (value == null || value === '') return '';
  var d = null;
  if (value instanceof Date) d = value;
  else if (typeof value === 'number' && isFinite(value)) d = new Date(value);
    ...
    var mUi = s.match(/^(\d{1,2}):(\d{2})\s+(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
    if (mUi) {
      return pad2_(Number(mUi[1])) + ':' + pad2_(Number(mUi[2])) + ' ' +
        pad2_(Number(mUi[3])) + '/' + pad2_(Number(mUi[4])) + '/' + mUi[5];
    }
    var mPretty = s.match(/^(\d{1,2}):(\d{2})\s*-\s*(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
    if (mPretty) {
      return pad2_(Number(mPretty[1])) + ':' + pad2_(Number(mPretty[2])) + ' ' +
        pad2_(Number(mPretty[3])) + '/' + pad2_(Number(mPretty[4])) + '/' + mPretty[5];
    }
    var mVn = s.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})(?:[ T](\d{1,2}):(\d{2})(?::(\d{2}))?)?/);
    if (mVn) {
...
/** Nhãn kho nổi bật trên header chi tiết (ưu tiên kho nhận) */
function resolveOrderKhoLabel_(khoNhan, khoXuat) {
  return formatStoreShortLabel_(khoNhan) || formatStoreShortLabel_(khoXuat) ||
    String(khoNhan || khoXuat || '').trim() || '—';
}

function pickOrderCreatedRaw_(obj) {
  if (!obj) return '';
  return obj.thoiGianTao || obj.thoiGianTaoPretty || obj.thoiGianPretty || obj.thoiGian ||
    obj.thoiGianTaoMs || obj.thoiGianMs || obj.createdAt || '';
}

function pickOrderUpdatedRaw_(obj) {
  if (!obj) return '';
  return obj.thoiGianCapNhat || obj.thoiGianCapNhatPretty || obj.thoiGianCapNhatMs ||
    obj.updatedAt || '';
}

/** HTML badge 2 dòng: Mã|Kho + [Ngày Tạo] - [Lần sửa cuối] */
function buildOrderMetaBadgeHtml_(soPhieu, khoLabel, createdRaw, updatedRaw) {
  var created = formatOrderTimestampUiClient_(createdRaw) || String(createdRaw || '').trim() || '—';
  var updated = formatOrderTimestampUiClient_(updatedRaw) || String(updatedRaw || '').trim() || created;
  return '<div class="om-line1"><span class="om-k">Mã Đơn:</span> ' + escapeHtml(soPhieu || '—') +
    '<span class="om-sep">|</span><span class="om-k">Kho:</span> ' + escapeHtml(khoLabel || '—') + '</div>' +
    '<div class="om-line2">[Ngày Tạo: ' + escapeHtml(created) + '] - [Lần sửa cuối: ' + escapeHtml(updated) + ']</div>';
}

function setOrderMetaBadgeEl_(elId, soPhieu, khoNhan, khoXuat, createdRaw, updatedRaw) {
  var el = document.getElementById(elId);
  if (!el) return;
  var kho = resolveOrderKhoLabel_(khoNhan, khoXuat);
  el.innerHTML = buildOrderMetaBadgeHtml_(soPhieu, kho, createdRaw, updatedRaw);
  el.style.display = '';
}

function refreshCreateOrderMetaBadge_() {
  var el = document.getElementById('create-order-meta-badge');
  if (!el) return;
  var kx = document.getElementById('select-kho-xuat');
  var kn = document.getElementById('select-kho-nhan');
...
/**
 * Header PDF sạch — KHÔNG banner xanh trùng.
 * 1) Đơn hàng  2) Ngày tạo | Cập nhật lần cuối  3) Kho xuất/nhận + địa chỉ
 */
function buildOrderPdfHeaderHtml_(soPhieu, thoiGianTaoRaw, khoXuat, khoNhan, thoiGianCapNhatRaw) {
  var so = escapeHtml(soPhieu || '');
  var tgTao = escapeHtml(formatOrderTimestampUiClient_(thoiGianTaoRaw) || thoiGianTaoRaw || '—');
  var tgCap = escapeHtml(
    formatOrderTimestampUiClient_(thoiGianCapNhatRaw) || thoiGianCapNhatRaw ||
    formatOrderTimestampUiClient_(thoiGianTaoRaw) || thoiGianTaoRaw || '—'
  );
...
function executePrintWeb(soPhieu, khoXuat, khoNhan, itemsArray) {
  var headerHtml = buildOrderPdfHeaderHtml_(
    soPhieu,
    (itemsArray && (itemsArray._thoiGianTao || itemsArray._thoiGian)) || '',
    khoXuat,
    khoNhan,
...
function buildOrderPdfHtml_(detail) {
  ...
  return '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Đơn hàng ' + so + '</title>' +
    ...
    buildOrderPdfSheetInnerHtml_(detail) +
    '</body></html>';
}
...
function confirm_loadPhieu(selectedSoPhieu) {
  ...
  apiGet('layDanhSachPhieuTheoFilter', { khoNhan: confirmStoreFilter, ngay: ngay, userRole: sessionUser.role, userStore: sessionUser.store }, { directOnly: true, timeoutMs: 45000 }).then(function(res) {
    try {
      ...
      selectEl.innerHTML = html;
      confirmPhieuList_ = confirmableOrders;
      if (selectedSoPhieu) {
...
function confirm_onSelectPhieu() {
  ...
  apiGet('getChiTietPhieu', { soPhieu: val, storeName: sessionUser.store, includeStock: '0' }, { directOnly: true, timeoutMs: 45000 }).then(function(res) {
    ...
    var confirmInfo = (confirmPhieuList_ || []).find(function(x) { return x && x.soPhieu === val; }) || {};
    setOrderMetaBadgeEl_(
      'confirm-order-meta-badge',
      val,
      confirmInfo.khoNhan || '',
      confirmInfo.khoXuat || '',
      pickOrderCreatedRaw_(confirmInfo),
      pickOrderUpdatedRaw_(confirmInfo)
    );
    currentConfirmPhieuObj.khoNhan = confirmInfo.khoNhan || '';
    currentConfirmPhieuObj.khoXuat = confirmInfo.khoXuat || '';
    currentConfirmPhieuObj.thoiGianPretty = pickOrderCreatedRaw_(confirmInfo);
    currentConfirmPhieuObj.thoiGianCapNhat = pickOrderUpdatedRaw_(confirmInfo);
    // Làm mới 2 mốc từ getThongTinPhieu (có audit soạn/sửa)
    apiGet('getThongTinPhieu', { soPhieu: val }, { timeoutMs: 20000 }).then(function(info) {
      if (!info || !info.soPhieu) return;
      setOrderMetaBadgeEl_(
        'confirm-order-meta-badge',
        val,
        info.khoNhan || confirmInfo.khoNhan,
        info.khoXuat || confirmInfo.khoXuat,
        pickOrderCreatedRaw_(info),
        pickOrderUpdatedRaw_(info)
      );
    }).catch(function() {});
    var serverMs = parsed.meta && parsed.meta._debugTotalMs;
...
function ql_hienThiChiTiet(phieu, options) {
  ...
  apiGet('getChiTietPhieu', { soPhieu: currentPhieuObj.soPhieu, storeName: currentPhieuObj.khoXuat, includeStock: '1' }, { directOnly: true, timeoutMs: 45000 }).then(function(res) {
    ...
    document.getElementById("ql-lbl-khoxuat").innerText = formatStoreShortLabel_(currentPhieuObj.khoXuat);
    document.getElementById("ql-lbl-khonhan").innerText = formatStoreShortLabel_(currentPhieuObj.khoNhan);
    setOrderMetaBadgeEl_(
      'ql-order-meta-badge',
      currentPhieuObj.soPhieu,
      currentPhieuObj.khoNhan,
      currentPhieuObj.khoXuat,
      pickOrderCreatedRaw_(currentPhieuObj),
      pickOrderUpdatedRaw_(currentPhieuObj)
    );
    apiGet('getThongTinPhieu', { soPhieu: currentPhieuObj.soPhieu }, { timeoutMs: 20000 }).then(function(info) {
      if (!info || !info.soPhieu) return;
      currentPhieuObj.thoiGian = info.thoiGian || currentPhieuObj.thoiGian;
      currentPhieuObj.thoiGianPretty = info.thoiGianPretty || info.thoiGianTao || currentPhieuObj.thoiGianPretty;
      currentPhieuObj.thoiGianMs = info.thoiGianMs || info.thoiGianTaoMs || currentPhieuObj.thoiGianMs;
      currentPhieuObj.thoiGianTao = info.thoiGianTao || info.thoiGianPretty;
      currentPhieuObj.thoiGianCapNhat = info.thoiGianCapNhat || '';
      currentPhieuObj.thoiGianCapNhatMs = info.thoiGianCapNhatMs || null;
      currentPhieuObj.createdAt = info.createdAt || info.thoiGianTaoMs;
      currentPhieuObj.updatedAt = info.updatedAt || info.thoiGianCapNhatMs;
      if (info.khoXuat) currentPhieuObj.khoXuat = info.khoXuat;
      if (info.khoNhan) currentPhieuObj.khoNhan = info.khoNhan;
      document.getElementById("ql-lbl-khoxuat").innerText = formatStoreShortLabel_(currentPhieuObj.khoXuat);
      document.getElementById("ql-lbl-khonhan").innerText = formatStoreShortLabel_(currentPhieuObj.khoNhan);
      setOrderMetaBadgeEl_(
        'ql-order-meta-badge',
...
function ql_isDuplicateOrderItem(item) {
  if (!item) return false;
  var itemMaHang = item.maHang || "";
  var itemMaVach = item.maVach || "";
  return editRows.some(function(existing) {
    if (!existing) return false;
    var existingMaHang = existing.maHang || "";
    var existingMaVach = existing.maVach || "";
    // Trùng khi cùng MH + MV (NFC normalize) — cho phép nhiều biến thể khác nhau cùng Parent
    if (itemMaHang && existingMaHang && itemMaVach && existingMaVach) {
      return cartCodesEqual_(itemMaHang, existingMaHang) && cartCodesEqual_(itemMaVach, existingMaVach);
    }
    if (itemMaHang && existingMaHang && cartCodesEqual_(itemMaHang, existingMaHang) && !itemMaVach && !existingMaVach) return true;
    if (itemMaVach && existingMaVach && cartCodesEqual_(itemMaVach, existingMaVach) && !itemMaHang && !existingMaHang) return true;
    return false;
  });
}
...
function ql_luuSua() {
  ...
  if (!beginSubmitBusy_('ql_luuSua', 'ql-btn-save', '⏳ Đang lưu...')) return;
  ...
  }).catch(function(err){ hideLoad(); endSubmitBusy_('ql_luuSua'); alert('Lỗi: '+err.message); });
}
...
function layItemsForOutput() {
  ...
  if (currentPhieuObj) {
    items._thoiGian = pickOrderCreatedRaw_(currentPhieuObj);
    items._thoiGianTao = pickOrderCreatedRaw_(currentPhieuObj);
    items._thoiGianCapNhat = pickOrderUpdatedRaw_(currentPhieuObj);
  }
  return items;
}
...
function sh_ensureWeekStart_() {
  if (!shWeekCalState_.weekStart) {
    shWeekCalState_.weekStart = formatDateInputValue(sh_mondayOf_(new Date()));
  }
  return shWeekCalState_.weekStart;
}

function sh_weekCalShift_(days) {
  var base = parseDateInputClient_(sh_ensureWeekStart_()) || sh_mondayOf_(new Date());
  base.setDate(base.getDate() + Number(days || 0));
  shWeekCalState_.weekStart = formatDateInputValue(sh_mondayOf_(base));
  sh_loadWeekCalendar_();
}

function sh_weekCalGoToday_() {
  shWeekCalState_.weekStart = formatDateInputValue(sh_mondayOf_(new Date()));
  sh_loadWeekCalendar_();
}

function parseDateInputClient_(yyyyMmDd) {
...
function sh_findOrderInWeekCal_(soPhieu) {
  var data = shWeekCalState_.data;
  if (!data || !data.days) return null;
  var target = String(soPhieu || '').trim();
  for (var i = 0; i < data.days.length; i++) {
    var day = data.days[i];
    var whs = (day && day.warehouses) || [];
...
function sh_buildWeekCalendarHtml_(res, mode, showPrint) {
  ...
  days.forEach(function(day) {
    var cellCls = 'sh-day-cell' + (day.isToday ? ' is-today' : '');
    var dateDisp = day.dateLabel || day.date || '';
    var printLabel = dateDisp || day.date;
    ...
    if (!whs.length) {
      ...
      whs.forEach(function(wh) {
        html += '<div class="sh-day-wh"><div class="sh-day-wh-label">' + escapeHtml(wh.khoLabel || wh.khoKey || 'Kho') + '</div>';
...
function sh_flattenDayOrders_(day) {
  var out = [];
  if (!day) return out;
  (day.warehouses || []).forEach(function(wh) {
    (wh.orders || []).forEach(function(ord) {
      out.push({
        soPhieu: ord.soPhieu,
        khoKey: wh.khoKey || '',
        khoLabel: wh.khoLabel || resolveOrderKhoLabel_(ord.khoNhan, ord.khoXuat),
        khoNhan: ord.khoNhan || '',
        khoXuat: ord.khoXuat || '',
        createdTime: ord.createdTime || '',
        thoiGianPretty: ord.thoiGianPretty || ord.thoiGian || '',
        createdAt: ord.createdAt || 0
      });
    });
  });
  out.sort(function(a, b) {
    var c = String(a.khoLabel || '').localeCompare(String(b.khoLabel || ''), 'vi');
    if (c) return c;
...
function sh_openPrintDayModal_(dateStr) {
  var data = shWeekCalState_.data;
  var day = ((data && data.days) || []).find(function(d) { return d && d.date === dateStr; });
  if (!day) return alert('Không tìm thấy ngày ' + dateStr);
  var orders = sh_flattenDayOrders_(day);
  if (!orders.length) return alert('Ngày này không có đơn để in.');
  shPrintDayState_ = {
    date: day.date,
  ...
  var title = document.getElementById('print-day-date-label');
  if (title) title.textContent = day.dateLabel || day.date;
  var filterEl = document.getElementById('print-day-wh-filter');
  if (filterEl) {
    var whMap = {};
    orders.forEach(function(o) { if (o.khoKey) whMap[o.khoKey] = o.khoLabel || o.khoKey; });
    var opts = '<option value="__all__">Tất cả các kho</option>';
    Object.keys(whMap).sort(function(a, b) {
      return String(whMap[a] || '').localeCompare(String(whMap[b] || ''), 'vi');
    }).forEach(function(k) {
      opts += '<option value="' + escapeHtml(k) + '">' + escapeHtml(whMap[k]) + '</option>';
    });
    filterEl.innerHTML = opts;
    filterEl.value = '__all__';
  }
  sh_printDayRenderList_(true);
  var modal = document.getElementById('modal-print-day');
  if (modal) modal.style.display = 'flex';
}

function sh_closePrintDayModal_() {
  var modal = document.getElementById('modal-print-day');
  if (modal) modal.style.display = 'none';
}

function sh_printDayApplyFilter_() {
  var filterEl = document.getElementById('print-day-wh-filter');
  shPrintDayState_.filterWh = filterEl ? filterEl.value : '__all__';
  sh_printDayRenderList_(true);
}

function sh_printDayVisibleOrders_() {
  var filter = shPrintDayState_.filterWh || '__all__';
  return (shPrintDayState_.orders || []).filter(function(o) {
    return filter === '__all__' || o.khoKey === filter;
  });
}

function sh_printDayRenderList_(tickAll) {
  var tbody = document.getElementById('print-day-tbody');
  if (!tbody) return;
  var rows = sh_printDayVisibleOrders_();
  if (!rows.length) {
    tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;color:#64748b;">Không có đơn theo bộ lọc kho.</td></tr>';
    return;
  }
  var html = '';
  rows.forEach(function(o, idx) {
    var checked = tickAll === false ? '' : ' checked';
    html += '<tr>' +
      '<td style="text-align:center;"><input type="checkbox" class="print-day-cb" data-sp="' + escapeHtml(o.soPhieu) + '"' + checked + '></td>' +
      '<td><b>' + escapeHtml(o.soPhieu) + '</b></td>' +
      '<td>' + escapeHtml(o.khoLabel || '—') + '</td>' +
      '<td>' + escapeHtml(o.createdTime || '—') + '</td></tr>';
  });
  tbody.innerHTML = html;
}

function sh_printDaySelectAll_(checked) {
  Array.from(document.querySelectorAll('.print-day-cb')).forEach(function(cb) {
    cb.checked = !!checked;
  });
}

function sh_printDaySelectedPdf_() {
  var selected = Array.from(document.querySelectorAll('.print-day-cb:checked')).map(function(cb) {
    return String(cb.getAttribute('data-sp') || '').trim();
  }).filter(Boolean);
  if (!selected.length) return alert('Chưa chọn đơn nào để in.');
  showLoad('Đang ghép ' + selected.length + ' đơn PDF...');
  var details = [];
  var chain = Promise.resolve();
  selected.forEach(function(sp) {
    chain = chain.then(function() {
      return apiGet('getOrderDetail', { soPhieu: sp }, { timeoutMs: 60000 }).then(function(res) {
        if (res && res.success) {
          details.push(res);
          return;
        }
        return apiGet('getChiTietPhieu', { soPhieu: sp, storeName: (sessionUser && sessionUser.store) || '', includeStock: '0' }, { timeoutMs: 45000 }).then(function(legacy) {
          var parsed = unwrapListResponse_(legacy);
          var rows = parsed.rows || [];
          var hit = (shPrintDayState_.orders || []).find(function(o) { return o.soPhieu === sp; }) || {};
          details.push({
            success: true,
            soPhieu: sp,
            khoXuat: hit.khoXuat || '',
            khoNhan: hit.khoNhan || '',
            thoiGianPretty: hit.thoiGianPretty || '',
            items: rows.filter(function(r) {
              return r && r.trangThai !== 'Đã hủy dòng' && r.trangThai !== 'Đã hủy đơn';
            }).map(function(r) {
              var sl = Number(r.slSoan) || Number(r.slThucTe) || Number(r.slGoc) || 0;
              return {
                maHang: r.maHang, maVach: r.maVach, tenHang: r.tenHang, dvt: r.dvt, sl: sl,
                parentSku: r.parentSku || '', variantSku: r.variantSku || r.maHang || '',
                variantName: r.variantName || r.tenHang || '', parentName: r.parentName || '',
                maHangDisplay: r.maHangDisplay || '', tenHangDisplay: r.tenHangDisplay || ''
              };
            })
          });
        });
  ...
  chain.then(function() {
    hideLoad();
    if (!details.length) return alert('Không tải được chi tiết đơn đã chọn.');
    openMultiOrderPdfWindow_(details);
  }).catch(function(err) {
    hideLoad();
    alert('Lỗi ghép PDF: ' + (err && err.message || err));
  });
}

function openMultiOrderPdfWindow_(detailsList) {
  var w = window.open('', '_blank');
  if (!w) {
    alert('Trình duyệt chặn tab mới. Cho phép popup rồi thử lại.');
    return;
  }
  var sheets = (detailsList || []).map(function(d) {
    return buildOrderPdfSheetInnerHtml_(d);
  }).join('');
  var title = 'In ' + (detailsList || []).length + ' đơn — ' + (shPrintDayState_.dateLabel || '');
  w.document.open();
...
function buildOrderPdfSheetInnerHtml_(detail) {
  var soRaw = String((detail && detail.soPhieu) || '').trim();
...
function sh_chonDonMobile() {
  ...
  var opt = document.getElementById("sh-phieu");
  var selectedOpt = opt && opt.options[opt.selectedIndex] ? opt.options[opt.selectedIndex] : null;
  var hintKho = selectedOpt ? String(selectedOpt.textContent || '') : '';
  setOrderMetaBadgeEl_('sh-order-meta-badge', sp, hintKho, '', '—', '—');
  apiGet('getChiTietDonHangMobile', { soPhieu: sp }, { directOnly: true, timeoutMs: 60000 }).then(function(items) {
    ...
    if (calHit) {
      setOrderMetaBadgeEl_(
        ...
        calHit.khoNhan,
        calHit.khoXuat,
        calHit.thoiGianTao || calHit.thoiGianPretty || calHit.thoiGian || calHit.createdAt,
        calHit.thoiGianCapNhat || calHit.updatedAt
      );
    }
    apiGet('getOrderDetail', { soPhieu: sp }, { timeoutMs: 30000 }).then(function(det) {
      if (det && det.success) {
        setOrderMetaBadgeEl_(
          'sh-order-meta-badge',
          det.soPhieu || sp,
...
function sh_luuPhieu() {
  ...
  if (!beginSubmitBusy_('sh_luuPhieu', 'sh-btn-save', '⏳ Đang lưu...')) return;
  ...
  }).catch(function(err){ hideLoad(); endSubmitBusy_('sh_luuPhieu'); alert('Lỗi: '+err.message); });
}
...
function adm_buildLockRowsFromCatalog_() {
  var seen = {};
  var rows = [];
  var keys = Object.keys(danhMucGoc || {});
  for (var i = 0; i < keys.length; i++) {
    var it = danhMucGoc[keys[i]];
    if (!it) continue;
    var mh = String(it.maHang || '').trim();
    var mv = String(it.maVach || '').trim();
    var dedupe = (mh || mv).toUpperCase();
    ...
    rows.push({
      ...
      dvt: it.dvt || '',
      parentSku: it.parentSku || '',
      isLocked: !!isProductLockedFlag_(it.isLocked),
      _origLocked: !!isProductLockedFlag_(it.isLocked)
    });
  }
  rows.sort(function(a, b) {
    return String(a.maHang || a.maVach).localeCompare(String(b.maHang || b.maVach), 'vi');
  });
  return rows;
}

function adm_loadProductLocks() {
  if (sessionUser.role !== 'Admin') return alert('Chỉ Admin được quản lý khóa đặt hàng.');
  var meta = document.getElementById('adm-lock-meta');
  if (!catalogLoadState.ready) {
    if (meta) meta.innerText = 'Danh mục đang tải — thử lại sau vài giây.';
    return;
  }
  admLockRows_ = adm_buildLockRowsFromCatalog_();
  adm_filterProductLocks_();
}

function adm_itemMatchesLockQuery_(it, qRaw) {
  return adm_itemMatchesNewProductQuery_(it, qRaw);
}

function adm_getFilteredLockIndexes_() {
  var qEl = document.getElementById('adm-lock-q');
  var qRaw = qEl ? String(qEl.value || '') : String(admLockFilterQ_ || '');
  admLockFilterQ_ = qRaw;
  var idxs = [];
  for (var i = 0; i < admLockRows_.length; i++) {
    if (adm_itemMatchesLockQuery_(admLockRows_[i], qRaw)) idxs.push(i);
  }
  return idxs;
}

function adm_filterProductLocks_() {
  var tbody = document.getElementById('adm-lock-tbody');
  var meta = document.getElementById('adm-lock-meta');
  if (!tbody) return;
  if (!admLockRows_.length) {
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:#64748b; padding:16px;">Nhấn "Tải danh sách" để quản lý khóa đặt hàng.</td></tr>';
    if (meta) meta.innerText = 'Chưa tải danh sách.';
    return;
  }
  var idxs = adm_getFilteredLockIndexes_();
  var lockedCount = 0;
  for (var c = 0; c < admLockRows_.length; c++) {
    if (admLockRows_[c].isLocked) lockedCount++;
  }
  if (meta) {
    meta.innerText = 'Hiển thị ' + idxs.length + '/' + admLockRows_.length +
      ' · Đang khóa: ' + lockedCount;
  }
  if (!idxs.length) {
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:#64748b; padding:16px;">Không khớp bộ lọc.</td></tr>';
    return;
  }
  var html = '';
  var maxShow = Math.min(idxs.length, 400);
  for (var i = 0; i < maxShow; i++) {
    var idx = idxs[i];
    var it = admLockRows_[idx];
    html += '<tr>' +
      '<td style="text-align:center;"><input type="checkbox" class="adm-lock-cb" data-idx="' + idx + '" ' +
      (it.isLocked ? 'checked' : '') + ' onchange="adm_onLockCheckboxChange_(this)"></td>' +
      '<td><b>' + escapeHtml(it.maHang || '-') + '</b></td>' +
      '<td>' + escapeHtml(it.maVach || '-') + '</td>' +
      '<td>' + escapeHtml(it.tenHang || '-') + (it.isLocked ? ' <span class="badge-locked">KHÓA</span>' : '') + '</td>' +
      '<td>' + escapeHtml(it.dvt || '-') + '</td></tr>';
  }
  if (idxs.length > maxShow) {
    html += '<tr><td colspan="5" style="text-align:center; color:#64748b;">… còn ' + (idxs.length - maxShow) + ' dòng (thu hẹp tìm kiếm)</td></tr>';
  }
  tbody.innerHTML = html;
}

function adm_onLockCheckboxChange_(el) {
  if (!el) return;
  var idx = Number(el.getAttribute('data-idx'));
  if (isNaN(idx) || !admLockRows_[idx]) return;
  admLockRows_[idx].isLocked = !!el.checked;
}

function adm_setVisibleProductLocks_(locked) {
  var idxs = adm_getFilteredLockIndexes_();
  for (var i = 0; i < idxs.length; i++) {
    admLockRows_[idxs[i]].isLocked = !!locked;
  }
  adm_filterProductLocks_();
}

function adm_saveProductLocks() {
  if (sessionUser.role !== 'Admin') return alert('Chỉ Admin được lưu khóa đặt hàng.');
  if (!admLockRows_.length) return alert('Chưa tải danh sách.');
  var items = [];
  for (var i = 0; i < admLockRows_.length; i++) {
    var row = admLockRows_[i];
    if (!!row.isLocked === !!row._origLocked) continue;
    items.push({
      maHang: row.maHang,
      maVach: row.maVach,
      tenHang: row.tenHang,
  ...
  if (!items.length) return alert('Không có thay đổi khóa để lưu.');
  if (!beginSubmitBusy_('adm_saveProductLocks', null, null)) return;
  showLoad('Đang lưu khóa đặt hàng (' + items.length + ')...');
  apiPost('updateProductLockStatus', {
    actor: sessionUser.user,
    items: items
  }, { directOnly: true, timeoutMs: 120000 }).then(function(res) {
    hideLoad();
    endSubmitBusy_('adm_saveProductLocks');
    if (!res || !res.success) {
      alert('❌ Lưu khóa thất bại: ' + ((res && (res.error || res.msg)) || 'unknown') + '\n[' + APP_BUILD + ']');
      return;
    }
    alert('✅ ' + (res.msg || ('Đã cập nhật ' + (res.updated || items.length) + ' sản phẩm.')) + '\n[' + APP_BUILD + ']');
    for (var j = 0; j < admLockRows_.length; j++) {
      admLockRows_[j]._origLocked = !!admLockRows_[j].isLocked;
    }
    reloadCatalogNow_();
  }).catch(function(err) {
    hideLoad();
    endSubmitBusy_('adm_saveProductLocks');
    alert('Lỗi lưu khóa: ' + (err && err.message || err) + '\n[' + APP_BUILD + ']');
  });
}
...
function analyzeImportRows(rows, importType) {
  ...
  var mapCols = impMapHeaderColumns_(header);
  var dvtColIdx = mapCols.dvt;
  if ((importType === 'stock' || importType === 'catalog') && dvtColIdx < 0) {
    warnings.push('Không thấy cột Đơn vị tính (ĐVT / ĐVT chính / Tên ĐVT) trong tiêu đề — sẽ không ghi ĐVT.');
  }
  if (mapCols.dvt2 >= 0 && mapCols.dvt >= 0) {
    // OK — ĐVT 2 tách riêng, không ghi đè ĐVT 1
  } else if (mapCols.dvt2 >= 0 && mapCols.dvt < 0) {
    warnings.push('Chỉ thấy cột ĐVT 2 / quy đổi — chưa nhận diện ĐVT chính.');
  }
  return {
    header: header,
...
function renderImportPreview(previewState) {
  ...
  var mapHint = '';
  try {
    var mapCols = impMapHeaderColumns_(previewState.analysis.header || []);
    if (mapCols && mapCols.labels) {
      mapHint = '<br><b>Map cột:</b> MaSP=[' + escapeHtml(mapCols.labels.maHang || '—') +
        '] · Tên=[' + escapeHtml(mapCols.labels.tenHang || '—') +
        '] · ĐVT1=[' + escapeHtml(mapCols.labels.dvt || '—') +
        '] · ĐVT2=[' + escapeHtml(mapCols.labels.dvt2 || '—') +
        '] · MV=[' + escapeHtml(mapCols.labels.maVach || '—') + ']';
      if (mapCols.dvt < 0) mapHint += ' <span style="color:#b91c1c;">⚠️ Chưa nhận diện cột ĐVT</span>';
    }
  } catch (eMap) {}
  meta.innerHTML = '<b>File:</b> ' + previewState.fileName + ' | <b>Sheet:</b> ' + previewState.sheetName + ' | <b>Dòng:</b> ' + previewState.analysis.rowCount + ' | <b>Cột:</b> ' + previewState.analysis.colCount + ' | <b>Nhận diện:</b> ' + (previewState.analysis.detectedType === 'stock' ? 'Tồn kho' : previewState.analysis.detectedType === 'catalog' ? 'Nhập khẩu thông tin' : 'Chưa rõ') + mapHint;
...
function impFindColByAliases_(header, aliases) {
  ...
  var a;
  var n;
  // Exact trước — tránh barcode khớp "code", unitname khớp "name"
  for (c = 0; c < header.length; c++) {
    n = impNormalizeText(header[c]).replace(/\s+/g, '');
    if (!n) continue;
    for (a = 0; a < aliases.length; a++) {
      if (n === aliases[a]) return c;
    }
  }
  for (c = 0; c < header.length; c++) {
    n = impNormalizeText(header[c]).replace(/\s+/g, '');
    if (!n) continue;
    for (a = 0; a < aliases.length; a++) {
      if (!aliases[a] || aliases[a].length < 4) continue;
...
function impHeaderNorm_(value) {
  return impNormalizeText(value).replace(/\s+/g, '');
}

function impIsDvt2HeaderNorm_(norm) {
  if (!norm) return false;
  var n = String(norm).replace(/\s+/g, '');
  if (n.indexOf('quydoi') !== -1) return true;
  if (n.indexOf('dvt2') !== -1 || n.indexOf('donvi2') !== -1) return true;
  if (n.indexOf('dvtphu') !== -1 || n.indexOf('donviphu') !== -1) return true;
  if (n.indexOf('unit2') !== -1 || n.indexOf('altunit') !== -1) return true;
  return false;
}

/** Điểm khớp header → vai trò (đồng bộ utils_helpers.scoreImportHeaderRole_) */
function impScoreHeaderRole_(norm, role) {
  if (!norm) return 0;
  var n = String(norm).replace(/\s+/g, '');
  if (role === 'parentSku') {
    if (n === 'parentsku' || n === 'parent' || n === 'manhomban') return 100;
    if ((n.indexOf('mahang') !== -1) && (n.indexOf('cha') !== -1 || n.indexOf('parent') !== -1)) return 95;
    return 0;
  }
  if (role === 'dvt2') {
    if (impIsDvt2HeaderNorm_(n)) return n.indexOf('quydoi') !== -1 ? 100 : 90;
    return 0;
  }
  if (role === 'dvt') {
    if (impIsDvt2HeaderNorm_(n)) return 0;
    if (n.indexOf('price') !== -1 || n.indexOf('gia') !== -1 || n.indexOf('madvt') !== -1) return 0;
    if (n === 'donvitinh' || n === 'dvtinh') return 100;
    if (n === 'tendvt' || n === 'dvtchinh' || n === 'donvichinh') return 98;
    if (n === 'dvt' || n === 'donvi') return 96;
    if (n === 'basicunit' || n === 'unitname' || n === 'uom' || n === 'unit') return 88;
    if (n.indexOf('donvitinh') !== -1) return 94;
    if (n.indexOf('tendvt') !== -1 || n.indexOf('dvtchinh') !== -1) return 92;
    if (n.indexOf('dvt') === 0 && n.length <= 12) return 80;
    return 0;
  }
  if (role === 'maHang') {
    if (n.indexOf('cha') !== -1 || n.indexOf('parent') !== -1) return 0;
    if (n.indexOf('mavach') !== -1 || n.indexOf('barcode') !== -1) return 0;
    if (n === 'mahanghoa' || n === 'masanpham') return 100;
    if (n === 'masp' || n === 'mahang' || n === 'mahh') return 96;
    if (n === 'itemcode' || n === 'article' || n === 'sku') return 90;
    if (n.indexOf('mahanghoa') !== -1 || n.indexOf('masanpham') !== -1) return 94;
    if (n.indexOf('mahang') !== -1 && n.indexOf('vach') === -1) return 88;
    return 0; // không dùng alias "code"
  }
  if (role === 'maVach') {
    if (n === 'mavach' || n === 'barcode' || n === 'barcodeid' || n === 'ean') return 100;
    if (n.indexOf('mavach') !== -1) return 95;
    if (n.indexOf('barcode') !== -1) return 90;
    return 0;
  }
  if (role === 'tenHang') {
    if (n.indexOf('dvt') !== -1 || n.indexOf('donvi') !== -1 || n.indexOf('unit') !== -1) return 0;
    if (n === 'tenhanghoa' || n === 'tensanpham') return 100;
    if (n === 'tensp' || n === 'tenhang') return 96;
    if (n === 'description') return 85;
    if (n === 'name') return 70;
    if (n.indexOf('tenhanghoa') !== -1 || n.indexOf('tensanpham') !== -1) return 94;
    if (n.indexOf('tenhang') !== -1 || n.indexOf('tensp') !== -1) return 90;
    return 0;
  }
  if (role === 'tonKho') {
    if (n.indexOf('nhapkho') !== -1 || n.indexOf('xuatkho') !== -1) return 0;
    if (n === 'tonkho' || n === 'soluongton' || n === 'slton' || n === 'cuoiky') return 100;
    if (n === 'soluong' || n === 'soton' || n === 'onhand' || n === 'stock') return 92;
    if (n.indexOf('tonkho') !== -1 || n.indexOf('soluongton') !== -1 || n.indexOf('soton') !== -1) return 90;
    if (n === 'qty' || n === 'quantity') return 75;
    if (n.indexOf('soluong') !== -1 && n.indexOf('quydoi') === -1) return 80;
    return 0;
  }
  return 0;
}

/** ĐVT hợp lệ (cái/hộp/thùng…) — loại số lượng / ngày / mã SP nhảy cột; giữ Đ/đ */
function impIsPlausibleDvt_(value) {
...
function impSanitizeDvt_(value) {
  var raw = String(value == null ? '' : value).trim();
  if (!raw) return '';
  if (raw.charAt(0) === "'") raw = raw.slice(1).trim();
  raw = raw.replace(/\u00A0/g, ' ').replace(/\s+/g, ' ').trim();
  return impIsPlausibleDvt_(raw) ? raw : '';
}

function impFindColByAliasesExclude_(header, aliases, excludeMap) {
  if (!header) return -1;
  var skip = excludeMap || {};
  var c, a, n;
  for (c = 0; c < header.length; c++) {
    if (skip[c]) continue;
    n = impHeaderNorm_(header[c]);
    if (!n) continue;
    for (a = 0; a < aliases.length; a++) {
      if (n === aliases[a]) return c;
    }
...
/** Map cột theo ĐIỂM SỐ tiêu đề — không hardcode index */
function impMapHeaderColumns_(header) {
  var out = { maHang: -1, tenHang: -1, dvt: -1, dvt2: -1, maVach: -1, parentSku: -1, tonKho: -1, labels: {}, scores: {} };
  ...
  for (c = 0; c < header.length; c++) {
    var n = impHeaderNorm_(header[c]);
    if (!n) continue;
    for (r = 0; r < roles.length; r++) {
      var role = roles[r];
      var sc = impScoreHeaderRole_(n, role);
      if (sc > best[role].score) best[role] = { idx: c, score: sc };
    }
  }
  var taken = {};
  var assignOrder = roles.slice().sort(function(a, b) {
  ...
  out.labels = {
    maHang: out.maHang >= 0 ? String(header[out.maHang] || '') : '',
    tenHang: out.tenHang >= 0 ? String(header[out.tenHang] || '') : '',
    dvt: out.dvt >= 0 ? String(header[out.dvt] || '') : '',
    dvt2: out.dvt2 >= 0 ? String(header[out.dvt2] || '') : '',
    maVach: out.maVach >= 0 ? String(header[out.maVach] || '') : '',
    tonKho: out.tonKho >= 0 ? String(header[out.tonKho] || '') : ''
  };
  return out;
}

function impIsJunkDataRow_(row, cols) {
  if (!row) return true;
  var cells = [];
  ...
  var joined = impHeaderNorm_(cells.join(' '));
  if (joined.indexOf('tongcong') !== -1 || joined === 'tong' || joined.indexOf('total') === 0) return true;
  if (joined.indexOf('congty') !== -1 && cells.length <= 3) return true;
  if (joined.indexOf('baocao') !== -1) return true;
  if (cols) {
    var look = 0;
    if (cols.maHang >= 0 && impScoreHeaderRole_(impHeaderNorm_(row[cols.maHang]), 'maHang') >= 70) look++;
    if (cols.dvt >= 0 && impScoreHeaderRole_(impHeaderNorm_(row[cols.dvt]), 'dvt') >= 70) look++;
    if (cols.tenHang >= 0 && impScoreHeaderRole_(impHeaderNorm_(row[cols.tenHang]), 'tenHang') >= 70) look++;
    if (look >= 2) return true;
  }
  return false;
}
...
function extractCatalogEntriesFromRows_(rows) {
  ...
  for (var hi = 0; hi < Math.min(rows.length, 15); hi++) {
    var mapped = impMapHeaderColumns_(rows[hi] || []);
    var score = 0;
    ...
    var nonEmpty = (rows[hi] || []).filter(function(cell) { return String(cell || '').trim(); }).length;
    score += Math.min(nonEmpty, 8);
    if (score > bestScore) { bestScore = score; headerIndex = hi; }
  }
  var header = rows[headerIndex] || [];
  var cols = impMapHeaderColumns_(header);
  ...
  var maHangIdx = cols.maHang;
  var maVachIdx = cols.maVach;
  var tenHangIdx = cols.tenHang;
  var dvtIdx = cols.dvt;
  var dvt2Idx = cols.dvt2;
  var parentIdx = cols.parentSku;
  var tonIdx = cols.tonKho;
  ...
  for (var r = startRow; r < rows.length; r++) {
    var row = rows[r];
    if (!row) continue;
    if (impIsJunkDataRow_(row, cols)) { skippedJunk++; continue; }
    ...
    var dRaw = dvtIdx >= 0 ? String(row[dvtIdx] == null ? '' : row[dvtIdx]).trim() : '';
    var d2Raw = dvt2Idx >= 0 ? String(row[dvt2Idx] == null ? '' : row[dvt2Idx]).trim() : '';
    var d = impSanitizeDvt_(dRaw);
    var d2 = impSanitizeDvt_(d2Raw);
    var p = parentIdx >= 0 ? String(row[parentIdx] == null ? '' : row[parentIdx]).trim() : '';
    if (!mh && !mv) continue;
    if (!normalizeProductCodeClient_(mh) && !normalizeProductCodeClient_(mv)) continue;
    if (dRaw && !d) skippedBadDvt++;
    if (d) withDvt++;
    if (d2) withDvt2++;
    if (p) withParent++;
    var tonRaw = tonIdx >= 0 ? String(row[tonIdx] == null ? '' : row[tonIdx]).trim() : '';
    var tonNum = tonRaw === '' ? NaN : Number(String(tonRaw).replace(/,/g, ''));

    function pushUnit_(unit) {
      var ent = { mh: mh, mv: mv, th: th, d: unit, dvt: unit, p: p };
      if (!isNaN(tonNum)) ent.ton = tonNum;
      ent.key = buildCatalogCompositeKeyClient_(mh, unit, mv, false);
      entries.push(ent);
      if (samples.length < 3) samples.push(ent);
    }
    if (d) pushUnit_(d);
    if (d2 && normalizeCatalogDvtPartClient_(d2) !== normalizeCatalogDvtPartClient_(d)) pushUnit_(d2);
    if (!d && !d2) pushUnit_('');
  }
  // Collision rule: cùng MaSP+ĐVT khác MV → thêm MaVach vào key
  var groups = {};
  for (var gi = 0; gi < entries.length; gi++) {
    var ge = entries[gi];
    var gb = String(ge.mh || '').trim() + '_' + normalizeCatalogDvtPartClient_(ge.d);
    if (!groups[gb]) groups[gb] = [];
    groups[gb].push(ge);
  }
  for (var gk in groups) {
    if (!Object.prototype.hasOwnProperty.call(groups, gk)) continue;
    var bucket = groups[gk];
    var mvSet = {};
    for (var bj = 0; bj < bucket.length; bj++) mvSet[String(bucket[bj].mv || '').trim()] = true;
    if (Object.keys(mvSet).length > 1) {
      for (var bk = 0; bk < bucket.length; bk++) {
        bucket[bk].key = buildCatalogCompositeKeyClient_(bucket[bk].mh, bucket[bk].d, bucket[bk].mv, true);
      }
    }
  }
  return {
...
function extractTonQ7EntriesFromRows_(rows) {
  ...
  var header = rows[headerIndex] || [];
  var stockCols = impMapHeaderColumns_(header);
  var maHangIdx = stockCols.maHang;
  var maVachIdx = stockCols.maVach;
  var dvtIdx = stockCols.dvt;
  var tenHangIdx = stockCols.tenHang;
  var tonIdx = stockCols.tonKho;
  if (tonIdx < 0) {
    tonIdx = impFindColByAliasesExclude_(header, ['tonkho', 'soluongton', 'stock', 'onhand', 'slton', 'cuoiky', 'soluong', 'soton'], {});
  }
  if (dvtIdx >= 0 && tonIdx === dvtIdx) tonIdx = -1;
  var q7Cols = [];
  for (var c = 0; c < header.length; c++) {
    if (c === maHangIdx || c === maVachIdx || c === dvtIdx || c === tonIdx || c === tenHangIdx || c === stockCols.dvt2) continue;
...
/** Admin: dọn ĐVT bất thường đã ghi sai lên sheet (one-time cleanup) */
function adm_fixWrongDVTOnSheet_() {
  if (!sessionUser || !sessionUser.user) return alert('Vui lòng đăng nhập Admin.');
  if (String(sessionUser.role || '') !== 'Admin') return alert('Chỉ Admin được phép dọn ĐVT sai.');
  if (!confirm('Quét Data_Excel + TON_VARIANT và XÓA các ô ĐVT bất thường (số lượng/ngày/mã SP nhảy cột)?\n\nSau đó hãy Import lại file MISA để ghi ĐVT chuẩn.')) return;
  showLoad('Đang dọn ĐVT sai trên sheet...');
  apiPost('fixWrongDVTOnSheet', {
    actor: sessionUser.user
  }, { directOnly: true, timeoutMs: 120000 }).then(function(res) {
    hideLoad();
    if (!res || !res.success) {
      alert('❌ Dọn ĐVT thất bại: ' + ((res && (res.error || res.msg)) || 'Không rõ lỗi') + '\n[Build: ' + APP_BUILD + ']');
      return;
    }
    var sampleText = '';
    if (res.samples && res.samples.length) {
      sampleText = '\n\nMẫu dòng đã xóa:\n' + res.samples.slice(0, 8).map(function(s) {
        return '- ' + (s.sheet || '') + ' R' + (s.row || '?') + ' [' + (s.maHang || '') + '] ← "' + (s.badDvt || '') + '"';
      }).join('\n');
    }
    alert('✅ ' + (res.msg || 'Xong.') +
      '\n- Data_Excel: ' + (res.clearedCatalog || 0) +
      '\n- TON_VARIANT: ' + (res.clearedVariant || 0) +
      sampleText + '\n[' + APP_BUILD + ']');
    clearCatalogLocalStorage();
    loadCatalogInBackground(true);
  }).catch(function(err) {
    hideLoad();
    alert('Lỗi: ' + (err && err.message || err) + '\n[Build: ' + APP_BUILD + ']\nclasp push rồi thử lại.');
  });
}

/** Admin: dọn Key trùng SKU vs SKU|ĐVT trên TON_VARIANT / TON_Q7 */
function adm_removeDuplicateStockRows() {
  if (!sessionUser || !sessionUser.user) return alert('Vui lòng đăng nhập Admin.');
  if (String(sessionUser.role || '') !== 'Admin') return alert('Chỉ Admin được phép dọn Key trùng.');
  if (!confirm('Dọn các dòng Key trùng trên TON_VARIANT / TON_Q7?\nGiữ 1 dòng chuẩn / sản phẩm (bỏ dạng AKV001|cai).')) return;
  showLoad('Đang dọn Key trùng tồn kho...');
  apiPost('removeDuplicateStockRows', {
    actor: sessionUser.user
  }, { directOnly: true, timeoutMs: 120000 }).then(function(res) {
    hideLoad();
    if (!res || !res.success) {
      alert('❌ Dọn trùng thất bại: ' + ((res && (res.error || res.msg)) || 'Không rõ lỗi') + '\n[Build: ' + APP_BUILD + ']');
      return;
    }
    alert('✅ Đã dọn Key trùng!\n' +
      '- TON_VARIANT: giữ ' + (res.variantKept || 0) + ', bỏ ' + (res.variantRemoved || 0) + '\n' +
      '- TON_Q7: giữ ' + (res.q7Kept || 0) + ', bỏ ' + (res.q7Removed || 0) + '\n' +
      (res.msg ? ('\n' + res.msg + '\n') : '') +
      '[' + APP_BUILD + ']');
  }).catch(function(err) {
    hideLoad();
    alert('Lỗi: ' + (err && err.message || err) + '\n[Build: ' + APP_BUILD + ']\nclasp push api_routes.gs + catalog_variant.gs rồi thử lại.');
  });
}
...
function importDanhMucTonKho() {
  ...
  if (!beginSubmitBusy_('importDanhMucTonKho', 'imp-submit-btn', '⏳ Đang cập nhật...')) return;
  ...
  if (importType === 'stock' && !wantFullStock) {
    ...
    try {
      ...
      hideLoad();
      endSubmitBusy_('importDanhMucTonKho');
    ...
    apiPost('nhapKhauCapNhatThongTin', {
      ...
      alert("✅ Cập nhật nhanh TON_Q7 thành công!\n" +
        ...
        ((res.dedupeVariantRemoved || res.dedupeQ7Removed)
          ? ("- Đã dọn Key trùng: variant " + (res.dedupeVariantRemoved || 0) + " / Q7 " + (res.dedupeQ7Removed || 0) + "\n")
          : "") +
        "- Thời gian: " + Math.round((Date.now() - t0) / 1000) + "s\n" +
        (res.msg ? ("\n" + res.msg + "\n") : "") +
        "[" + APP_BUILD + " / import-q7-key-canon]");
  ...
  if (importType === 'catalog') {
    ...
    if (!catExtracted.entries.length) {
      hideLoad();
      endSubmitBusy_('importDanhMucTonKho');
      var why = (catExtracted.meta && catExtracted.meta.reason) || '';
      alert("Không tách được dòng hàng từ file nhập khẩu thông tin.\n" +
        (why === 'missing_ma_sp_or_barcode'
          ? "Không nhận diện được cột Mã hàng / Mã vạch theo tiêu đề.\n"
          : "Kiểm tra có cột Mã hàng / Mã vạch / Tên / ĐVT.\n") +
        "Map: " + JSON.stringify((catExtracted.meta && catExtracted.meta.labels) || {}));
      return;
    }
    if (catExtracted.meta && catExtracted.meta.dvtIdx < 0) {
      if (!confirm("⚠️ Không nhận diện được cột ĐVT trong file.\nVẫn tiếp tục import (ĐVT sẽ để trống)?")) {
        hideLoad();
        endSubmitBusy_('importDanhMucTonKho');
        return;
      }
    }
    // Xác nhận map cột trước khi ghi sheet
    var lab = (catExtracted.meta && catExtracted.meta.labels) || {};
    var samp = (catExtracted.meta && catExtracted.meta.samples) || [];
    var sampText = samp.map(function(s, i) {
      return (i + 1) + ') MH=' + (s.mh || '-') + ' | MV=' + (s.mv || '-') + ' | Tên=' + (s.th || '-') + ' | ĐVT=' + (s.d || '-');
    }).join('\n');
    if (!confirm(
      'Xác nhận map cột trước khi ghi Sheet:\n' +
      '- MaSP ← [' + (lab.maHang || '—') + ']\n' +
      ...
      '- ĐVT2 ← [' + (lab.dvt2 || '—') + ']\n' +
      '- MaVach ← [' + (lab.maVach || '—') + ']\n' +
      '- Số dòng: ' + catExtracted.entries.length +
      ((catExtracted.meta.skippedJunk || 0) ? (' (bỏ rác: ' + catExtracted.meta.skippedJunk + ')') : '') + '\n\n' +
      'Mẫu 3 dòng đầu:\n' + (sampText || '(không có)') + '\n\nTiếp tục upload?'
    )) {
      hideLoad();
      endSubmitBusy_('importDanhMucTonKho');
    ...
    sendCatChunk(0).then(function(res) {
      ...
      var labels = (catExtracted.meta && catExtracted.meta.labels) || {};
      alert("✅ UPSERT danh mục thành công (map cột động theo header)!\n" +
        "- Staging: MISA_IMPORT\n" +
        ...
        "- Map: MaSP=[" + (labels.maHang || '—') + "] ĐVT1=[" + (labels.dvt || '—') + "] MV=[" + (labels.maVach || '—') + "]\n" +
        "- Có ĐVT hợp lệ: " + (catExtracted.meta.withDvt || 0) +
        (dvtOk ? (" (cột #" + (catExtracted.meta.dvtIdx + 1) + ")") : " (⚠️ không thấy cột ĐVT)") +
        ((catExtracted.meta.skippedBadDvt || 0) ? ("\n- Bỏ ĐVT bất thường: " + catExtracted.meta.skippedBadDvt) : "") + "\n" +
        "- Chunk: " + catChunks.length + "\n" +
        "- Thời gian: " + Math.round((Date.now() - t0) / 1000) + "s\n" +
        (res.msg ? ("\n" + res.msg + "\n") : "") +
        "[" + APP_BUILD + " / import-dvt-header-map]");
      if (fileEl) fileEl.value = '';
      importPreviewState = null;
      renderImportPreview(null);
      clearCatalogLocalStorage();
...
/** Chống double-submit: disable nút + spinner text trong lúc request */
var submitBusyMap_ = {};
function beginSubmitBusy_(key, btnId, busyLabel) {
  if (submitBusyMap_[key]) return false;
  var btn = btnId ? document.getElementById(btnId) : null;
  submitBusyMap_[key] = { btnId: btnId || '', prevHtml: btn ? btn.innerHTML : '', prevDisabled: btn ? !!btn.disabled : false };
  if (btn) {
    btn.disabled = true;
...
function endSubmitBusy_(key) {
  var st = submitBusyMap_[key];
  delete submitBusyMap_[key];
  if (!st) return;
  var btn = st.btnId ? document.getElementById(st.btnId) : null;
  if (!btn) return;
  btn.disabled = !!st.prevDisabled;
  btn.removeAttribute('aria-busy');
  if (st.prevHtml) btn.innerHTML = st.prevHtml;
}
...
function xb_handleSearchInput(e) {
  ...
  if (e && e.key === "Enter") {
    var exactMatch = resolveProductFromScanQuery_(val);
    if (exactMatch) xb_chonSanPhamByKey_(encodeURIComponent(String(exactMatch.key || ensureItemCatalogKey_(exactMatch))));
    else {
...
function xb_chonSanPhamByKey_(keyEnc) {
  var product = resolveSelectedCatalogProductByKey_(keyEnc);
  if (!product) return alert('Không tìm thấy quy cách trong danh mục.');
  xb_chonSanPham(product);
...
function xb_chonSanPhamByMaSP_(maHangEnc, maVachEnc) {
  var mh = decodeURIComponent(String(maHangEnc || ''));
  var mv = decodeURIComponent(String(maVachEnc || ''));
  var product = resolveSelectedCatalogProduct_(mh, mv);
  if (!product) return alert('Không tìm thấy sản phẩm [' + (mh || mv || '?') + '] trong danh mục.');
  xb_chonSanPham(product);
  var input = document.getElementById("xb-input-scan");
  var box = document.getElementById("xb-suggest-box");
  if (input) { input.value = ""; input.focus(); }
...
function xb_chonSanPhamFromSuggest(itemStr) {
  try {
    var parsed = JSON.parse(decodeURIComponent(itemStr));
    if (parsed.key) {
      xb_chonSanPhamByKey_(encodeURIComponent(String(parsed.key)));
      return;
    }
    xb_chonSanPhamByMaSP_(
      encodeURIComponent(String(parsed.maHang || '')),
      encodeURIComponent(String(parsed.maVach || ''))
    );
  } catch (e) {
...
function xb_chonSanPham(it) {
  ...
  var resolved = (it && it.key ? resolveSelectedCatalogProductByKey_(it.key) : null) ||
    resolveSelectedCatalogProduct_(it && it.maHang, it && it.maVach, it && it.dvt) ||
    cloneCatalogProduct_(it) || it;
  ...
  xb_chonSanPhamDirect_(resolved, 1);
}

function xb_chonSanPhamDirect_(it, qty) {
  if (!xbInvoiceLocked) return alert("Xác nhận số hóa đơn liên kết trước.");
  if (!it) return;
  var resolved = (it.key ? resolveSelectedCatalogProductByKey_(it.key) : null) ||
    resolveSelectedCatalogProduct_(it.maHang, it.maVach, it.dvt) ||
    cloneCatalogProduct_(it) || it;
  if (!guardProductOrderable_(resolved, 'xb-input-scan')) return;
  var addQty = Number(qty);
  if (!addQty || addQty <= 0 || isNaN(addQty)) addQty = Number(it.sl) > 0 ? Number(it.sl) : 1;
  var maHang = String(resolved.maHang || "");
  var maVach = String(resolved.maVach || "");
  var tenHang = resolved.tenHang || "";
  var dvt = String(resolved.dvt || it.selectedDVT || it.dvt || "").trim();
  var resolvedKey = resolved.key || ensureItemCatalogKey_(resolved);
  var existingIndex = xbItems.findIndex(function(x) {
    if (x.maHang === "LỖI MÃ") return false;
    var xKey = x.key || buildCatalogCompositeKeyClient_(x.maHang, x.selectedDVT || x.dvt, x.maVach, false);
    if (resolvedKey && xKey && resolvedKey === xKey) return true;
    return cartCodesEqual_(x.maHang, maHang) &&
      cartCodesEqual_(x.maVach, maVach) &&
      normalizeCatalogDvtPartClient_(x.selectedDVT || x.dvt) === normalizeCatalogDvtPartClient_(dvt);
  });
  if (existingIndex !== -1) {
    ...
    xbItems[existingIndex].selectedDVT = xbItems[existingIndex].dvt;
    xbItems[existingIndex].key = resolvedKey;
    xbItems[existingIndex].maVach = maVach;
    ensureCartLineId_(xbItems[existingIndex]);
  } else {
    var row = {
      lineId: '',
      key: resolvedKey,
      ...
      selectedDVT: dvt,
      tonKho: resolved.tonKho,
      parentSku: resolved.parentSku || it.parentSku || '',
      sl: String(addQty),
      highlight: true
    };
    ensureCartLineId_(row);
...
function xb_thayDoiSoLuong(lineId, delta) {
  var index = findCartItemByLineId_(xbItems, lineId);
  if (index < 0) return;
...
function xb_setCartQtyByLineId_(lineId, value) {
  var index = findCartItemByLineId_(xbItems, lineId);
  if (index < 0) return;
  xbItems[index].sl = value;
  xb_renderTable();
}

function xb_removeCartItemByLineId_(lineId) {
  var index = findCartItemByLineId_(xbItems, lineId);
  if (index < 0) return;
  xbItems.splice(index, 1);
  xb_renderTable();
}

function xb_onChangeUnit(lineId, newDvt) {
  var index = findCartItemByLineId_(xbItems, lineId);
  if (index < 0) return;
  var dvt = String(newDvt || '').trim();
  var cur = xbItems[index];
  var match = resolveSelectedCatalogProduct_(cur.maHang, '', dvt) ||
    resolveSelectedCatalogProduct_(cur.maHang, cur.maVach, dvt);
  if (match) {
    xbItems[index].key = match.key || buildCatalogCompositeKeyClient_(match.maHang, match.dvt, match.maVach, false);
    xbItems[index].maHang = String(match.maHang || cur.maHang || '');
    xbItems[index].maVach = String(match.maVach || cur.maVach || '');
    xbItems[index].tenHang = match.tenHang || cur.tenHang;
    xbItems[index].dvt = String(match.dvt || dvt);
    xbItems[index].selectedDVT = xbItems[index].dvt;
    if (match.tonKho != null) xbItems[index].tonKho = match.tonKho;
  } else {
    xbItems[index].dvt = dvt;
    xbItems[index].selectedDVT = dvt;
    xbItems[index].key = buildCatalogCompositeKeyClient_(cur.maHang, dvt, cur.maVach, false);
  }
}
...
function xb_renderTable() {
  ...
  xbItems.forEach(function(it, i) {
    ...
    it.highlight = false;
    it.dvt = resolveDvtClient_(it.maHang, it.maVach, it.selectedDVT || it.dvt) || it.dvt;
    it.selectedDVT = it.dvt;
    var dvtCell = isErr ? escapeHtml(it.dvt || '—') : buildCartDvtSelectHtml_(it, lineId, 'xb_onChangeUnit');
    var lid = String(lineId).replace(/\\/g, '\\\\').replace(/'/g, "\\'");
    tbody.insertAdjacentHTML("beforeend",
      '<tr class="' + trClass + '" data-line-id="' + escapeHtml(lineId) + '"><td>' + (xbItems.length - i) + '</td>' +
      '<td>' + formatPackingCodeHtml_(it) + '</td>' +
      '<td style="font-weight:500;">' + escapeHtml(formatPackingProductName_(it)) + '</td><td>' + dvtCell + '</td>' +
      '<td><div class="qty-control"><button class="qty-btn" onclick="xb_thayDoiSoLuong(\'' + lid + '\', -1)">-</button>' +
      '<input type="number" class="qty-input" value="' + it.sl + '" onchange="xb_setCartQtyByLineId_(\'' + lid + '\', this.value)">' +
      '<button class="qty-btn" onclick="xb_thayDoiSoLuong(\'' + lid + '\', 1)">+</button></div></td>' +
      '<td style="text-align:center;"><button style="color:#d93025; border:none; background:none; font-weight:bold; cursor:pointer; font-size:18px;" onclick="xb_removeCartItemByLineId_(\'' + lid + '\')">×</button></td></tr>');
  });
...
function xb_submit() {
  ...
  if (!beginSubmitBusy_('xb_submit', null, null)) return;

  var itemsToSave = xbItems.map(function(it) {
    var cat = lookupCatalogProductClient_(it.maHang, it.maVach);
    return {
      maHang: String((cat && cat.maHang) || it.maHang || ''),
      maVach: String((cat && cat.maVach) || it.maVach || ''),
      tenHang: (cat && cat.tenHang) || it.tenHang,
      dvt: resolveDvtClient_(it.maHang, it.maVach, it.selectedDVT || it.dvt),
      sl: it.sl
    };
  });

  showLoad("Đang lưu xuất bán...");
  // Disable cả 2 nút lưu XB trên UI
  document.querySelectorAll('#tab-xuat-ban .btn-submit').forEach(function(b) {
    if (!b._xbPrev) b._xbPrev = b.innerHTML;
    b.disabled = true;
    b.innerHTML = '⏳ Đang lưu...';
  });
  apiPost("luuXuatBanHang", {
    ...
    hideLoad();
    endSubmitBusy_('xb_submit');
    document.querySelectorAll('#tab-xuat-ban .btn-submit').forEach(function(b) {
      b.disabled = false;
      if (b._xbPrev) { b.innerHTML = b._xbPrev; delete b._xbPrev; }
    });
...
function buildInvoicePdfHtml_(detail) {
  ...
  var cn = escapeHtml(formatStorePrintLabel_(detail.chiNhanh) || formatStoreShortLabel_(detail.chiNhanh) || detail.chiNhanh || "-");
